...
foreach($page_links as $page_link)
{
    ...
    foreach($card_page_links as $card_page_link)
    {
        ...
        echo $card_properities[1]."\n";
        echo $card_properities[10]."\n";
...
class CardsParser
{
    ...
    function get_card_properities($card_url)
    {
        ...
        $from = array(' .',' ');
        $to = array('.','%20');
        $img_url = str_replace($from,$to,$img_url);
...
'".$card_properities[10]."')";
    ...
    {
        $content = file_get_contents($fileurl) or die("fail: ".$fileurl);
        $destFileName = "Hearth_Stone_Pictures/".$filename.".png";
        $fp = fopen($destFileName, 'wb');
        fwrite($fp,$content);
/*
        $storage = new SeaStorage();
        $domain = 'lushipic';
        $attr = array('encoding'=>'gzip');
        $result = $sotrage->write($domain,$filename, $content, -1, $attr, true);
        if($result)
        {
            echo "保存成功！！";
        }else{
